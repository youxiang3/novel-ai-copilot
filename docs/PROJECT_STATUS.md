...
- API 改稿流式返回：当前已有同步 API 改稿、取消生成和 120 秒超时保护，但还没有流式增量展示。
- API 改稿复杂历史版本管理：已有后端日志和前端历史面板 V0，但还没有分页、筛选、删除、成本统计、失败原因统计和正式版本对比。
- 真实模型 Agent 多步任务编排：已有本地助手搜索 / 安全动作 V0 和后端助手基础，但还不能稳定执行跨步骤自动工作流。
- 跨作品搜索：当前搜索仍主要围绕当前作品。
- IP Factory 资产管理：尚未建立统一资产库、版本、来源、章节/角色/场景关联和下载管理。
- 生图能力：封面、角色卡、场景图、分镜参考图生成仍未接入。
- 可运行游戏工程导出：当前只有互动剧情游戏设定包方向，还不能导出可运行工程。
- 前端章节 ID 与后端 chapterId 完整映射：API 改稿日志能写入 `novelId`，但 `chapterId` 仍可能为空。
...
- 当前是同步请求 V0，暂未做流式返回、取消生成、重试队列和模型调用日志落库增强。
...
本轮补齐智能助手的本地安全操作雏形：在写作工作台右侧新增“本地助手搜索”，可以在当前作品内搜索章节、资料库、长篇记忆、检查问题和本地产物记录，并提供打开定位或插入修改区等需要用户确认的操作。
