...
## 2026-07-10 智能助手 Agent 改造决策与换机交接

智能助手已提升为下一阶段最高优先级。已确认采用“轻量 Agent Core + Novel Tools”的渐进式方案：先修通真实登录态端到端流程，再统一模型接口、Tool 注册、有限步骤执行、写操作确认和执行记录；暂不提前建设多 Agent、插件市场和复杂工作流平台。

同时保留两条后期 Codex 协作路径：Codex 通过 CLI/Skill/MCP 使用小说系统能力，以及小说助手通过独立 Codex Adapter 调度单个工程任务。Skill 第一版只支持本地受信任 `SKILL.md`，Plugin 仅预留边界。

完整未完成清单、实施顺序和第一阶段完成标准见 `docs/ASSISTANT_AGENT_HANDOFF.md`。

## 2026-07-10 产品重构与下一章任务卡 V0

产品主线已调整为“面向中文网文作者的长篇创作管理、内容诊断和 Agent 协作平台”。完整方案见 `docs/PRODUCT_RESTRUCTURE_PLAN.md`。

本轮 UI 收口：

- 写作工作台脱离全局 App Shell，进入全宽专注模式，避免全局导航、顶部搜索和工作台三栏重复占用横向空间。
- 全局导航从 12 项压缩为 9 项，删除重复的工具箱、提示词库和弱相关入口，“创作台”改名为“新建作品”。
- 工作台左栏资料分类压缩为资料库、伏笔线索和长篇记忆；具体分类继续在资料库内部筛选。
- 右栏快捷动作从 8 项压缩为摘要、记忆、一致性和追读 4 项，其余能力保留在对应页签。
- 写作工作台隐藏重复的全局智能助手，只保留绑定当前作品状态的本地助手搜索。
- 增加 1180px 响应式断点：宽屏保持三栏，窄屏切换为可滚动单列；900px 复测无横向溢出。

本轮完成：

- 新增 `ChapterTask` 正式前端数据结构，任务卡随 `SavedWork` 保存并进入现有作品版本快照。
- 写作工作台新增“下一章任务卡”视图，支持核心目标、情绪目标、目标字数、故事线、卷内节点、标题候选、必须完成、禁止事项和节奏步骤。
- 支持任务卡待确认、进行中、已完成状态流转。
- 支持根据作品资料、章节结尾和已确认记忆生成当前章任务卡草稿。
- 支持“一次创建下一章并生成任务卡”，创建后先确认任务，再进入正文。
- Web AI 改稿、检查和记忆 Prompt 已加入本章任务卡约束。
- 新增后端 `chapter_version` 独立版本模型，记录章节、版本号、来源、变更摘要、父版本和关联 AI 调用。
- 新增章节版本创建、列表、详情和恢复 API；恢复前自动保存当前正文为 `restore-backup`，恢复结果再记录为新版本。
- 完成章节版本前端闭环：保存当前章节会创建 `manual-save` 版本，手动“留版本”会创建 `manual-snapshot` 版本，确认采用 AI 改稿会同步正文并创建 `ai-adopt` 版本。
- 写作工作台新增“当前章节版本”面板，支持刷新、来源与摘要展示、正文预览和安全恢复；恢复操作会同步更新编辑器正文。
- 作品快照保存和云端载入会保留前端章节 ID 与后端 `backendChapterId` 映射，确保后续保存、改稿和版本操作绑定正确章节。
- 章节恢复流程增加事务保护，自动备份、正文更新和恢复版本记录作为一个整体提交。
- 新增长篇记忆 `memory_entry` 后端模型，支持事件、人物状态、世界事实、未回收伏笔、章节摘要和禁止规则六类记忆。
- 长篇记忆支持待确认、已确认、已拒绝、可能过期四态，并记录置信度、创建来源、原文证据、来源章节、审核时间和过期时间。
- 新增 `/api/memories` 列表、筛选、详情、创建、更新和删除接口，所有操作校验当前账号的作品所有权及来源章节归属。
- 写作工作台支持读取后端记忆、批量同步本地候选，以及即时同步确认、拒绝、过期和删除操作；后端不可用时继续保留本地内容。
- 新增 `diagnostic_run` / `diagnostic_issue` 统一诊断模型，记录作品、章节、诊断类型、运行模式、输入快照、评分、风险计数和创建时间。
- 追读、OOC / 伏笔一致性、人味修订、爆款潜力和 Web AI 检查统一保存问题类型、严重程度、位置、原文证据、原因、建议、优先级、置信度和处理状态。
- 新增 `/api/diagnostics` 运行创建、历史筛选、详情、删除和问题状态更新接口，所有接口校验作品、章节和当前账号所有权。
- 写作工作台“证据化章节诊断”新增当前章节诊断历史，支持打开旧运行，以及同步忽略和已解决状态；未同步作品继续使用本地诊断兜底。
- 新增后端 `chapter_task` 独立任务卡模型，按章节保存当前版本和完整历史版本，并记录父版本、状态、来源和来源依据。
- 新增 `/api/chapter-tasks` 作品当前任务卡、章节当前版本、版本历史、创建新版本和删除接口，所有操作校验作品及章节所有权。
- 写作工作台支持读取当前章节后端任务卡、显示最近版本、显式“保存任务卡”创建新版本，以及任务状态切换自动形成后端版本。
- 本地任务卡新增 `backendTaskId`、版本号、父版本和来源依据；未同步章节继续保留本地兜底，避免伪报云端保存成功。
- 新增 `chapter_workflow_state` 章节创作阶段版本链，统一记录任务规划、正文写作、章节诊断、章节改稿、正文确认、记忆确认、发布准备和本章完成八个阶段。
- 新增 `/api/chapter-workflow` 当前阶段、阶段历史和阶段转换接口，记录触发来源、转换原因、父状态及关联任务卡 / 诊断 / 版本 / 记忆对象，并拦截非法直接跳转。
- 写作工作台新增八阶段进度带和手动阶段选择；任务卡进行中、诊断落库、插入修改建议、采用 AI 改稿、确认记忆和标记发布会自动推进对应阶段。

接下来最高优先级：

- P0 真实流程修复：按浏览器点击、请求、数据库、刷新恢复重新验收任务卡、版本、诊断、记忆、工作流和 API 改稿。
- P0 轻量 Agent：统一 `ModelProvider`、`AssistantTool`、`ToolRegistry`、有限步骤 `AssistantRunner` 和执行记录，先完成单助手 + Novel Tools。
- P1 模型化：正式接入 MemoryExtractionSkill、ConsistencyCheckSkill 和诊断模型输出，复用已完成的记忆 / 诊断后端结构。
- P1 助手统一：合并全局模型助手与工作台本地搜索助手，接入本地受信任 Skill，所有写操作保留用户确认。
- P2 工程协作：助手调用 Codex 的单任务闭环，以及 Codex 通过标准项目导出、`AGENTS.md`、CLI、Skill、MCP 使用小说系统能力。
- P2 云端能力：实时同步、冲突检测、差异合并和跨设备编辑保护。
- P3 外部生态：IP 资产库、生图、分镜和可运行游戏工程导出。

## 2026-07-09 上传远端前未完成记录

任务 ID：`NOVELAI-COPILOT-REMOTE-UPLOAD-HANDOFF-20260709`

本次上传远端前，当前仍未完成或仍需增强的事项：

- 完整云同步和冲突合并：已有本地保存、云端快照和版本记录基础，但仍缺少实时/完整云同步、冲突检测、差异合并和跨设备同一作品编辑保护。
- API 改稿流式返回：后端已新增 `/api/chapters/rewrite/stream`，前端 API 改稿优先读取流式响应并展示增量文本；当前为 V0，完成后仍进入“采用文本”、版本卡片、diff 和二次确认，不自动覆盖正文。
- API 改稿复杂历史版本管理：已有后端日志、前端历史面板 V1、本地版本对比 V0、费用换算 V0、失败原因统计 V0、后端失败调用落库 V0、标准错误码 V0、供应商 usage 优先写入 V0 和后端跨版本对比 V0，支持分页加载、按当前章节筛选、删除单条记录、总数、token 统计、快照间章节级 diff、最近调用 token 条形图、按手动单价换算费用、已加载日志风险/异常信号统计、模型调用失败标记与错误码分类；流式或供应商未返回 usage 时仍回退为估算 token，供应商价格表自动同步和真实账单对账尚未接入。
- 真实模型 Agent 多步任务编排：已有本地助手搜索 / 安全动作 V0、跨作品搜索 V0 和后端助手基础，但还不能稳定执行跨步骤自动工作流。
- 跨作品搜索增强：本地跨作品标题、简介、章节检索 V0 已接入；仍缺后端语义搜索、跨作品关系图谱和 Agent 自动引用能力。
- IP Factory 资产管理：尚未建立统一资产库、版本、来源、章节/角色/场景关联和下载管理。
- 生图能力：封面、角色卡、场景图、分镜参考图生成仍未接入。
- 可运行游戏工程导出：当前只有互动剧情游戏设定包方向，还不能导出可运行工程。

本次额外补齐：

- API 改稿支持取消生成和 120 秒超时保护，取消或超时时不会写入采用文本、产物记录或正文。
- 前端章节 ID 与后端 `chapterId` 映射闭环：作品快照章节新增 `backendChapterId`，后端保存 / 版本历史 / 云端恢复保留真实章节 ID，云端备份成功后前端回写 `backendNovelId` 与章节 `backendChapterId`，API 改稿请求会携带当前章节的后端 ID。
- API 改稿流式返回 V0：复用现有用户模型配置和 `AiService.streamCall`，前端流式展示模型原始输出，结束后解析为后端改稿版本并写入历史日志。

## 2026-07-09 API 改稿历史面板 V0

任务 ID：`NOVELAI-COPILOT-BACKEND-REWRITE-HISTORY-UI-V0`

本轮把后端 `chapter_rewrite` 调用日志接入前端：写作工作台右侧新增“API 改稿历史”面板，可刷新最近的后端 API 改稿记录，并从历史记录中再次填入“采用文本”。再次采用仍然只进入 diff 预览和二次确认替换，不会直接覆盖正文。

已完成：

- 新增 `BackendRewriteLog` 类型和 `backendRewriteLogs` 状态。
- 正式作品有 `backendNovelId` 且已登录时，会读取 `GET /api/chapters/rewrite/logs?novelId=...`。
- 新增 `BackendRewriteLogPanel`，展示最近 API 改稿时间、mode/model、估算 token 和响应预览。
- 新增“再次采用”，会从 `responseSnapshot` 解析推荐文本并填入“采用文本”。
- 历史响应不是标准 JSON 时，会按原始文本兜底填入，并保留风险提示。

边界说明：

- 当前历史面板只展示最近记录，不做复杂筛选、分页、删除和成本统计图。
- 当前仍依赖后端正式作品 `novelId`；纯本地草稿没有后端历史。
- 再次采用不自动覆盖正文，仍需用户检查 diff 后确认替换。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-09 API 改稿调用日志 V0

任务 ID：`NOVELAI-COPILOT-BACKEND-REWRITE-AI-LOG-V0`

本轮继续补后端 API 改稿的可追溯能力：当正式作品携带后端 `novelId` 调用 API 改稿时，后端会把 prompt、模型响应、workflow、估算 token 和创建时间写入既有 `ai_generation_log` 表；本地草稿仍可正常调用 API 改稿，不会因为没有后端作品 ID 阻断生成。

已完成：

- `ChapterRewriteRequest` 新增 `novelId` / `chapterId`。
- `ChapterRewriteSkill` 在模型调用后写入 `ai_generation_log`，workflowType 为 `chapter_rewrite`。
- 日志记录 prompt 快照、response 快照、mode/model 标识、估算 token 和创建时间。
- 日志写入失败不会影响改稿结果返回。
- 前端 API 改稿请求会携带 `work.backendNovelId`。
- 新增 `GET /api/chapters/rewrite/logs?novelId=...`，可查询指定作品的 API 改稿日志。

边界说明：

- 当前 chapterId 仍可能为空，因为前端多章节本地结构尚未完整映射后端 chapterId。
- 当前 tokenUsage 是按文本长度估算，不是模型供应商返回的真实 token usage。
- 当前前端还没有日志列表 UI，接口已预留给后续历史版本 / 调用审计面板。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。
- 已运行 `backend` 下 `mvn -q -DskipTests compile`，通过。

## 2026-07-09 API 改稿多版本采用 V0

任务 ID：`NOVELAI-COPILOT-BACKEND-REWRITE-VERSION-ADOPT-V0`

本轮补齐后端 API 改稿的采用体验：模型接口返回的推荐采用、保守改、扩写、精修和强刺激改会转成前端版本卡片，和本地候选版本一起展示。用户可以一键把任意 API 版本填入“采用文本”，再继续走字数差异、逐词 diff 和二次确认替换。

已完成：

- 新增 `backendRewriteOptions` 状态，保存 API 改稿返回的多版本候选。
- 新增 `buildBackendRewriteVersionOptions`，把后端结果转成统一的 `RewriteVersionOption`。
- 多版本卡片区同时展示 API 候选和本地候选，并用标签区分来源。
- 选区变化时自动清空旧 API 候选，避免误采用上一段文本的模型结果。
- 采用 API 版本仍只是填入“采用文本”，不直接覆盖正文。

边界说明：

- 当时仍未做流式返回、取消生成和调用日志落库；后续已补取消 / 超时、调用日志、历史面板和流式返回 V0。
- 当前 API 候选只保存在当前页面状态和产物记录中，还不是完整历史版本管理。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-09 后端模型 API 改稿 V0

任务 ID：`NOVELAI-COPILOT-BACKEND-REWRITE-API-V0`

本轮接入后端真实模型 API 改稿闭环：写作工作台的改稿条新增“API 改稿”，登录并保存模型配置后，可以把当前章节、选中文本、资料库和长篇记忆发给后端模型接口生成多版本改稿。模型结果不会自动覆盖正文，只会填入“采用文本”，继续走字数差异、逐词 diff 和二次确认替换。

已完成：

- 后端新增 `ChapterRewriteRequest` / `ChapterRewriteResult`。
- 后端新增 `ChapterRewriteSkill`，复用现有 `AiService` 和用户模型配置，调用 OpenAI-compatible `/chat/completions`。
- 后端新增 `POST /api/chapters/rewrite`。
- 前端新增“API 改稿”按钮，调用成功后把推荐采用文本填入“采用文本”。
- API 改稿结果写入本地产物记录，保留推荐文本、保守改、扩写、精修、强刺激改和风险提示。
- 仍保持安全边界：不自动覆盖正文，不跳过替换确认。

边界说明：

- 当前需要登录并保存后端模型配置；未登录或模型配置不可用时，会提示继续使用本地改稿或 Web AI。
- 当时是同步请求 V0；后续已补流式返回 V0、取消 / 超时和模型调用日志落库，重试队列仍未完成。
- 当前不自动解析并展示 API 返回的全部版本卡片，先把推荐版本填入采用文本，完整内容保存在产物记录。
...
## 2026-07-09 智能助手搜索 / 操作小说 V0

任务 ID：`NOVELAI-COPILOT-LOCAL-ASSISTANT-SEARCH-ACTIONS-V0`

本轮补齐智能助手的本地安全操作雏形：在写作工作台右侧新增“本地助手搜索”，可以在当前作品内搜索章节、资料库、长篇记忆、检查问题和本地产物记录，并提供打开定位或插入修改区等需要用户确认的操作。后续已扩展跨作品搜索 V0，可检索作品库内其它作品的标题、简介和章节内容，并从结果中打开目标作品。

已完成：

- 新增 `buildAssistantSearchResults`，统一检索章节、资料、记忆、检查问题和产物记录。
- 新增 `LocalAssistantSearchPanel`，在右侧创作任务栏展示本地搜索入口和结果列表。
- 搜索结果支持“打开”定位：章节跳到编辑器，资料跳到资料库，其它类型跳到对应视图。
- 对资料、记忆、检查建议和产物记录提供“插入修改区”，不直接覆盖正文。
- 跨作品搜索 V0 已接入：写作工作台接收作品库列表，搜索结果可展示“作品”和“跨作品章节”，打开后跳转到目标作品。
- 搜索结果限制最多 8 条，避免右侧栏过长。

边界说明：

- 当前是本地搜索和安全动作 V0，不是后端真实模型 Agent。
- 当前不会自动改写正文，不会自动修改资料库或长篇记忆。
- 当前还没有自然语言任务编排、多步计划执行、后端语义搜索和跨作品自动引用。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-09 多版本改稿采用 V0

任务 ID：`NOVELAI-COPILOT-REWRITE-VERSION-ADOPT-V0`

本轮继续补“改稿采用”的安全感：在正文选区下方提供本地多版本候选，用户可以先比较保守改、扩写、精修和强刺激改，再一键把某个版本填入“采用文本”，继续走字数差异、逐词 diff 和二次确认替换。

已完成：

- 新增 `buildRewriteVersionOptions`，根据当前选区和改稿要求生成 4 个本地候选版本。
- 候选版本覆盖：保守改、扩写、精修、强刺激改。
- 每个版本显示版本定位、候选字数和相对原选区的字数变化。
- 新增“采用”按钮，一键把对应版本填入“采用文本”输入框。
- 采用后复用已有字数差异预览、逐词 diff 预览和替换选区二次确认。

边界说明：

- 当前候选版本是本地规则生成，不是后端真实模型 API 改稿。
- 当前“一键采用”只是填入待确认文本，不会直接覆盖正文。
- 当前已有本地快照版本对比 V0；还不是完整的后端跨版本持久化对比。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-09 逐词 diff 高亮 V0

任务 ID：`NOVELAI-COPILOT-REWRITE-WORD-DIFF-HIGHLIGHT-V0`

本轮继续补“改稿采用”的安全感：在用户输入“采用文本”后，除了字数变化外，编辑器底部会展示逐词 diff 预览，替换前可以直观看到删除和新增内容。

已完成：

- 新增 `buildRewriteDiffPreview`，基于轻量 LCS 计算原选区和采用文本的差异。
- 新增 `tokenizeRewriteDiffText`，中文按字、英文数字按词、标点和空白独立参与预览。
- “采用文本”输入后展示 diff 预览：红色删除、绿色新增、灰色保留。
- 长文本预览限制为前 900 个片段，避免大选区导致前端卡顿。

边界说明：

- 当前仍不是多版本改稿对比。
- 当前仍不是一键采纳某个生成版本；仍需要用户手动粘贴采用文本并确认替换。
- 当前仍不接真实模型 API。
...
## 2026-07-08 改稿差异预览 V0

任务 ID：`NOVELAI-COPILOT-REWRITE-DIFF-PREVIEW-V0`

本轮继续增强改稿采用流程：在替换选区前显示原选区和采用文本的字数变化，并把差异统计写入本地替换记录。当前仍不是完整逐词 diff，也不是多版本对比。

已完成：

- “采用文本”输入后展示原选区字数、采用文本字数和增减字数。
- 当字数变化过大时，显示轻量风险提示，提醒替换前确认节奏和设定。
- 选区替换记录中新增字数变化摘要。
- 替换记录 metadata 新增 `originalCharCount`、`replacementCharCount`、`charDelta`。
- 新增 `countTextChars` 统一统计文本字数，忽略空白字符。

边界说明：

- 当前不是逐词 diff 高亮。
- 当前已有本地快照版本对比 V0；还不是完整后端多版本对比界面。
- 当前仍需要用户手动确认替换。

验证：

- 已运行 `frontend` 中 `npm run build`，通过。

## 2026-07-08 选区替换确认 V0

任务 ID：`NOVELAI-COPILOT-SELECTION-REPLACE-CONFIRM-V0`

本轮补齐改稿采用的安全闭环：用户选中正文片段后，可以把确认采用的改稿文本粘贴到“采用文本”输入框，再通过二次确认替换选区。当前仍不自动采用生成结果，不做无确认覆盖。

已完成：

- 正文选中文本后，编辑器底部显示“采用文本”输入框。
- 新增“替换选区”按钮，只有存在选区和采用文本时可执行。
- 替换前会二次确认。
- 替换前会校验当前选区内容是否仍匹配，避免正文变化后替错位置。
- 替换后会写入本地产物记录，记录原选区和采用文本。
- 替换完成后清空选区状态和采用文本。

边界说明：

- 当前不是自动采纳 AI 结果；需要用户手动粘贴采用文本并确认。
- 当前已有字数差异预览；还没有逐词 diff 高亮、多版本对比和一键采纳。
- 当前仍不接真实模型 API。
...
## 2026-07-08 选中文本级改稿 V0

任务 ID：`NOVELAI-COPILOT-SELECTED-TEXT-REWRITE-V0`

本轮补齐改写 / 扩写 / 精修的选中文本级目标能力。用户在正文编辑器中选中一段文字后，本地改稿包和 Web AI 改稿 Prompt 都会优先围绕这段选中文本生成建议；当前仍只插入修改区，不自动替换选区。

已完成：

- 正文编辑器支持捕获当前选中的文本片段。
- 编辑器底部改稿条会显示已选中字数。
- 本地 `buildLocalRewritePackage` 支持传入选中文本，输出中明确标记改稿范围。
- Web AI 改稿 Prompt 会携带选中文本，并要求外部模型优先改写该片段。
- Web AI 改稿回填记录会保存选中文本，方便后续追溯。
- 右侧“改写 / 扩写 / 精修”入口也会复用当前选中文本。

边界说明：

- 当前已有“确认后替换选区”V0，但不会自动采纳生成结果。
- 当前没有多版本对比和一键采纳。
- 当前不接真实模型 API，Web AI 仍是复制 / 粘贴回填流程。

验证：

- 已运行 `frontend` 中 `npm run build`，通过。

## 2026-07-08 Web AI 改稿 Prompt 回填 V0

任务 ID：`NOVELAI-COPILOT-WEBAI-REWRITE-ROUNDTRIP-V0`

本轮补齐改写 / 扩写 / 精修的 Web AI 轻量闭环：用户可以从编辑器复制改稿 Prompt 到外部模型，粘贴返回结果后回填到本地修改区和产物记录。当前仍不接真实模型 API，不自动覆盖正文。

已完成：

- Web AI Prompt 目标从“检查 / 记忆”扩展为“检查 / 记忆 / 改稿”。
- 编辑器底部新增 `Web AI` 按钮，可基于当前章节和改稿要求打开改稿 Prompt。
- 改稿 Prompt 会带入作品题材、卖点、简介、标签、资料库、长篇记忆和当前选中章节正文。
- 改稿 Prompt 要求外部模型输出保守改、扩写、精修、强刺激改和作者确认清单。
- Web AI 改稿返回内容粘贴解析后，会插入正文末尾【修改区】。
- Web AI 改稿返回内容会写入本地产物记录，类型为 `rewrite`。
- PromptRun 本地记录支持记录改稿 Prompt 的复制和解析。
- 顺手修正 Web AI Prompt 使用当前选中章节正文，而不是只依赖作品兼容字段。

边界说明：

- 当前仍是复制到 ChatGPT / DeepSeek / Gemini 等外部网页模型的半自动流程。
- 当前不接后端模型 API，不读取第三方网页结果。
- 当前不自动替换正文，只插入修改区。
- 当前还没有做自动替换选区、多版本对比和一键采纳。

验证：

- 已运行 `frontend` 中 `npm run build`，通过。

## 2026-07-08 改写 / 扩写 / 精修 V0

任务 ID：`NOVELAI-COPILOT-REWRITE-EXPAND-POLISH-V0`

本轮继续完成优先级中的“改写 / 扩写 / 精修工作流”。当前为本地规则版，不接真实模型 API，不自动替换正文，只生成可确认的改稿包并插入正文末尾【修改区】。

已完成：

- 编辑器底部原“扩写”输入条接入真实本地功能，改为“生成改稿”。
- 右侧“创作任务”新增“改写 / 扩写 / 精修”入口。
- 新增 `buildLocalRewritePackage` 本地规则函数，会根据用户输入的改稿要求选择关注片段。
- 支持识别开篇、章末、压缩、爽点、冲突等改稿意图。
- 输出四类改稿方向：
  - 保守改：不改变剧情，增强动作、停顿、视线和人物反应。
  - 扩写：补场景、动作、对话、心理和代价。
  - 精修：减少说明腔、拆短句、增强推进。
  - 强刺激改：增强冲突、反差、代价、爽点或章末钩子。
- 改稿结果写入本地产物记录，新增 `rewrite` 产物类型。
- 改稿结果只插入【修改区】，不覆盖原正文。
...
- 当前是本地规则改稿包，不是模型润色。
- 当前不做自动全文改写，不做批量洗稿。
- 当前不自动替换正文，用户需要自行确认后采用。
- 当前没有接入 Web AI 回填解析和后端模型 API。

验证：

- 已运行 `frontend` 中 `npm run build`，通过。

## 2026-07-08 爆款潜力诊断 V0

任务 ID：`NOVELAI-COPILOT-VIRAL-POTENTIAL-DIAGNOSIS-V0`

本轮新增“爆款潜力诊断 V0”。当前定位是网文编辑视角的本地规则诊断工具，只基于作品资料、当前章节文本、资料库和已确认记忆做结构化修稿判断；不接真实模型 API，不接平台数据，不输出真实平台结果，不自动改写正文。

已完成：

- 写作工作台右侧“创作任务”新增“爆款潜力诊断”入口。
- 检查页新增“爆款诊断”和“深度诊断 Prompt”按钮。
- 新增 `localViralPotentialDiagnosis` 本地规则函数，输出摘要层和 `CheckIssue` 诊断卡片。
- 诊断覆盖题材清晰度、卖点强度、开篇钩子、主角目标、冲突密度、爽点兑现、章末追读、IP 衍生潜力、AI 味 / 模板化风险。
- 每条问题补充 `dimension`、`level`、`title`、`evidence`、`excerpt`、`reason`、`suggestion`、`suggestionText`、`priority`、`confidence`、`source` 等字段。
- 诊断摘要包含 `diagnosisVersion: viral-potential-local-v0`、`diagnosisMode: local-rules`、`overallLevel`、`overallScore`、`dimensionScores`、风险计数和固定免责声明。
- UI 明确展示“当前为本地规则评分，仅作创作参考”。
- 诊断结果复用检查卡片的复制建议、插入修改区、标记已处理和忽略能力。
- 新增深度诊断 Prompt 复制能力，用于外部 ChatGPT / DeepSeek / Gemini 等模型做人工确认后的深度分析。
- 新增单项改写 Prompt，从具体 `CheckIssue` 生成针对性改写要求。
- 诊断完成后写入本地产物记录，记录诊断版本、模式、章节标题、风险计数、摘要、Prompt 和 issues。

边界说明：

- 当前是本地规则版，不是商业预测模型。
- 当前没有真实模型调用，没有平台数据，没有排行榜 / 热榜抓取。
- 当前不承诺平台推荐结果、真实商业表现或检测器判断。
- 当前不自动改写正文，所有建议都需要作者复制、插入或手动确认后使用。
- IP 衍生潜力在 V0 中只作为附加观察，不作为总分主要依据。
- 后续可升级为 Web AI Prompt 版、后端模型 API 版、长篇记忆增强版和平台数据增强版。

验证：

- 已运行 `frontend` 中 `npm run build`，通过。

## 2026-07-08 路线优先级与差异化能力重排

任务 ID：`NOVELAI-COPILOT-PRIORITY-DIFFERENTIATION-ROADMAP-P1`

本轮根据新的产品要求，重排未完成事项优先级，并把短剧、互动剧情游戏、智能助手搜索 / 操作、生图、爆款预测、改写和去模板腔能力正式写入路线图。

已完成：

- `docs/ROADMAP.md` 新增“2026-07-08 优先级重排与差异化定位”。
- 明确市面 AI 小说 / 写作助手常见能力基线：续写、改写、润色、大纲、世界观、长文本、搜索 / Agent、生图和自动化。
- 明确 NovelAI Copilot 差异化方向：
  - 爆款预测。
  - 追读诊断。
  - 改写 / 扩写 / 精修。
  - 去模板腔 / 人味修订。
  - 长篇记忆。
  - IP Factory。
- 明确短剧和互动剧情游戏已经排入路线，归入 `IP Factory`：
  - P1.5：短剧脚本草案、互动剧情游戏设定包。
  - P2：漫剧分镜、角色视觉、场景图、封面、生图资产管理。
- 明确智能助手能力拆成三层：
  - 搜索层：作品、章节、设定、记忆、检查结果、产物记录。
  - 操作层：打开、定位、插入建议、生成摘要、提取记忆、运行检查、保存版本。
  - 代理层：编排多步任务，但写入前必须用户确认。
- 明确生图排入 P2，优先服务封面、角色卡、场景氛围图、短剧 / 漫剧分镜参考，不优先做泛图片社区。
- `docs/PRD_NOVELAI_COPILOT.md` 更新产品定位、核心流程、核心方向和 P1 / P2 优先级。
- `docs/FEATURE_FLAGS_AND_PLACEHOLDERS.md` 增加爆款预测、智能助手搜索 / 操作、生图、追读诊断、人味修订、IP Factory 的状态文案规则。
...
6. Memory V0 增强。
7. 智能助手搜索和操作小说。
8. IP Factory 产品化：短剧 / 互动剧情游戏。
9. 生图能力：封面 / 角色 / 场景 / 分镜参考。

边界说明：

- 本轮是产品路线和状态文档更新，不是新增代码功能。
- 爆款预测不会承诺真实平台概率。
- 去模板腔不等同于绕过 AI 检测。
- 生图暂未接入真实模型。
- 短剧 / 游戏剧情已有 MVP 入口，但完整资产管理、视频合成和可运行游戏导出未完成。

验证：

- 本轮只修改文档，未运行前端构建。

## 2026-07-08 Prompt 工作流补充要求 P1

任务 ID：`NOVELAI-COPILOT-PROMPT-WORKFLOW-INSTRUCTION-P1`

本轮继续收口 Prompt 轻量工作流：在 Web AI Prompt 弹窗里补充“本次额外要求”，让用户可以在复制前追加临时指令，而不是只能复制固定上下文。

已完成：

- Web AI Prompt 弹窗新增“本次额外要求”文本框。
- 额外要求只影响本次弹窗内的合成 Prompt，不保存为模板。
- 复制 Prompt 时会复制合成后的最终 Prompt。
- Prompt Inspector V0 的字符数、估算 token 和风险提示会基于合成后的 Prompt 重新计算。
- PromptRun 本地记录会记录实际复制的合成 Prompt 字符数和估算 token。

边界说明：

- 当前不是正式提示词模板库。
- 当前不做多版本 Prompt 管理。
- 当前仍不接真实模型调用，Web AI 结果需要用户粘贴并确认。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 长篇记忆候选提取 V0 P1.5

任务 ID：`NOVELAI-COPILOT-MEMORY-V0-CANDIDATE-EXTRACT-P1.5`

本轮继续推进 Memory V0 前置能力：不实现完整 MemoryExtractionSkill，不接真实模型，只补齐“从当前章节生成可确认记忆候选”的本地闭环。

已完成：

- 写作工作台右侧“创作任务”新增“提取记忆候选”卡片。
- 长篇记忆页新增“提取记忆候选”按钮。
- 新增 `buildLocalMemoryCandidates` 本地规则函数，会从当前章节生成 4 类候选：
  - 已发生事件。
  - 人物状态变化。
  - 世界观事实。
  - 未回收伏笔。
- 候选条目默认 `status: draft`，不会自动变成已确认记忆。
- 每条候选都会记录 `sourceChapterId`、`sourceText`、`confidence`、`createdBy: local` 和创建时间。
- 用户可以继续编辑候选内容，再点击“确认 / 标记过期 / 拒绝”。
- 提取结果会写入本地产物记录，方便复制、插入或下载。

边界说明：

- 当前是本地规则候选提取，不是完整 MemoryExtractionSkill。
- 当前不会自动写入“已确认”记忆，必须用户确认。
- 当前不接后端记忆库，不做向量召回。
- 世界观事实和伏笔候选是轻量启发式判断，复杂抽取留到后续模型能力。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 人味修订 V0 P1

任务 ID：`NOVELAI-COPILOT-HUMAN-TASTE-REVISION-V0-P1`

本轮补齐“人味修订 V0”的本地规则入口，定位为减少模板腔、增强作者表达和网文口感，不叫“反 AI 检测”，不承诺规避任何检测。

已完成：

- 写作工作台右侧“创作任务”新增“人味修订”卡片。
- 检查页新增“人味修订”按钮。
- 新增 `localHumanTasteRevision` 本地规则函数，围绕当前章节生成修订建议卡。
- 输出三种修订方向：
  - 保守改：不改变剧情，增加动作、停顿和具体反应。
  - 强刺激改：增加误判、私心、欲望、选择代价和人性张力。
  - 爽文改：增加短句交锋、压迫、反击和对方失态节点。
- 会识别章节是否有对话、身体细节、内心矛盾和模板化表达，并调整建议风险等级。
- 修订建议复用检查页现有能力：复制建议、插入修改区、忽略、标记已处理。
- 人味修订结果会写入本地产物记录，后续可复制、插入或下载。

边界说明：

- 当前是本地规则建议，不是模型润色，不自动改正文。
- 当前不做批量洗稿，不承诺规避 AI 检测。
- 三种方向需要用户选择其一或局部采用，不建议全部叠加。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 追读诊断 V0 P1

任务 ID：`NOVELAI-COPILOT-READTHROUGH-DIAGNOSIS-V0-P1`

本轮补齐网文向“追读诊断 V0”的本地规则入口，不接真实平台数据，不承诺预测真实追读率，只把当前章节转成可处理的修稿诊断卡片。

已完成：

- 写作工作台右侧“创作任务”新增“追读诊断”卡片。
- 检查页新增“追读诊断”按钮，可和原 OOC / 伏笔本地检查并列使用。
- 新增 `localReadthroughDiagnosis` 本地规则函数，围绕当前章节生成 `CheckIssue` 卡片。
- 诊断覆盖：
  - 开头 300 字是否有问题、欲望、冲突或异常。
  - 本章目标是否清晰。
  - 冲突压力是否包含对抗、期限、损失或情绪压力。
  - 爽点兑现是否有反击、揭示、获得或关系推进。
  - 信息密度是否偏说明腔。
  - 章末 300 字是否有未解决问题、反转、危险或下一章诱因。
- 诊断结果复用检查页现有卡片能力：复制建议、插入修改区、忽略、标记已处理。
- 追读诊断结果会写入本地产物记录，后续可复制、插入或下载。

边界说明：

- 当前是本地规则修稿清单，不是平台后台数据分析。
- 当前不预测真实追读率，不自动覆盖正文。
- 当前仍不接真实模型；更细的追读诊断增强留到后续结合长篇记忆、章节目标、故事线和读者反馈。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 章节统计联动收口 P0.5

任务 ID：`NOVELAI-COPILOT-CHAPTER-STATS-LINK-P0.5`

本轮继续收口作品库 / 作品总览 / 写作工作台之间的章节统计口径：章节编辑、删除、发布 / 撤回后，作品级字数、章节数、最近更新时间和本月更新章节尽量从章节数组实时重算，避免旧作品字段残留造成展示不一致。

已完成：

- 写作工作台归一化作品时，会从 `chapters` 实时重算作品总字数。
- 写作工作台归一化作品时，会从 `chapters.length` 实时重算章节数。
- 写作工作台归一化作品时，会从 `status === 'published'` 的章节数量重算本月更新章节。
- 章节正文、标题、删除、移动、发布 / 撤回、批量发布 / 批量退回、批量删除后，`applyWorkspaceChapters` 会统一刷新 `words`、`chapterCount`、`monthlyUpdatedChapters` 和 `updatedAt`。
- 主应用保存 / 同步 / 版本快照前的 `withChapterStats` 也改为同一口径：本月更新章节来自已发布章节数量，而不是只增不减的旧值。

边界说明：

- 当前“本月更新章节”仍是前端轻量统计，等同于当前作品中已标记发布的章节数，不是真实按自然月聚合的后端发布统计。
- 当前不接真实平台发布数据，也不做跨月份历史归档。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 存储迁移设计收口 P1

任务 ID：`NOVELAI-COPILOT-STORAGE-MIGRATION-DESIGN-P1`

本轮把存储迁移文档和当前登录后端迁移行为对齐，并补上本地正式作品上传后的后端 ID 回写，避免“已上传到账号但本地作品仍不知道真实后端 novelId”的断点。

已完成：

- `docs/STORAGE_AND_MIGRATION.md` 从纯规划更新为“本地优先 + 登录后端快照”的迁移期说明。
- 文档明确当前已实现的迁移行为：登录后读取后端作品、弹出迁移确认、只上传正式作品、临时草稿不自动上传、失败保留 localStorage。
- 文档明确当前尚未实现的范围：迁移记录表、多端冲突合并、批次回滚、Storyline / ToolState / PromptRun / MemoryItem 全量后端化、同步详情。
- “上传正式作品”迁移路径改为等待后端保存结果，并把返回的 `backendNovelId` 合并回本地作品。
- 当前打开的作品如果刚好是被上传的正式作品，也会同步更新本地 `activeWork`，方便后续资料库同步定位后端小说。
- 上传结果会提示尝试数量和成功数量；临时草稿继续仅保存在本地。

边界说明：

- 当前仍是作品快照迁移，不是最终规范化迁移。
- 当前不删除 localStorage，不做自动冲突合并，不自动上传临时草稿。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 云端版本历史 MVP P2

任务 ID：`NOVELAI-COPILOT-CLOUD-VERSION-HISTORY-MVP-P2`

本轮继续把“云同步 / 版本历史 / 跨设备恢复”的未完成项推进到可验收 MVP：在已有手动云端快照基础上，每次保存作品快照都会额外写入一条云端版本记录，前端导出 / 备份中心可以查看版本列表并按版本恢复为新的本地副本。

已完成：

- 后端新增 `work_snapshot_version` 表，用于保存作品快照历史版本。
- 后端新增 `WorkSnapshotVersion`、`WorkSnapshotVersionMapper`、`WorkSnapshotVersionResponse`。
- `POST /api/work-library` 保存或更新当前作品快照时，会同步写入一条版本历史。
- 版本历史保存内容包含：前端作品 ID、作品标题、完整 payload、章节数组、章节数、字数、创建时间。
- 同一账号同一前端作品默认保留最近 20 个云端版本，避免无限增长。
- 后端新增 `GET /api/work-library/versions`，支持按 `frontendWorkId` 查询当前账号版本历史。
- 导出 / 备份中心的“查看云端快照”改为读取云端版本历史。
- 云端版本列表展示标题、版本时间、章节数、字数，并支持选择任意版本恢复为新的本地作品副本。
- 恢复仍不会覆盖当前作品，保持“手动恢复为副本”的安全策略。

边界说明：

- 当前仍不是实时多端协同，不做自动冲突合并。
- 当前是手动云端版本历史，不是完整 Git 式差异版本管理。
- 当前跨设备恢复依赖登录账号和后端服务可用；后端不可用时仍保留本地 JSON 备份 / 导入路径。

验证：

- 已使用 JDK 21 运行 `backend` 下 `mvn -DskipTests compile`，通过。
- 已运行 `frontend` 下 `npm run build`，通过。
- 已启动后端 `http://localhost:8080`。
- 已通过本地 HTTP 烟测完成注册、保存同一作品两次、查询 `GET /api/work-library/versions?frontendWorkId=...`，返回 2 条版本历史，且章节数 / 字数可区分版本。

## 2026-07-08 云端备份与恢复快照 P1

任务 ID：`NOVELAI-COPILOT-CLOUD-BACKUP-SNAPSHOT-P1`

本轮继续处理导出 / 备份中心里的显性未完成项：原“云端备份”只提示接口预留。现在登录用户可以从备份中心直接写入后端作品快照，并从当前账号最近的云端快照恢复为新的本地作品副本，作为跨设备恢复和后续云同步的基础能力。

已完成：

- `ExportCenterPage` 新增 `token` 入参，主应用会把当前后端登录 token 传入导出中心。
- “云端备份”按钮改为调用 `POST /api/work-library`。
- 云端备份 payload 包含：
  - 前端作品 ID。
  - 作品标题。
  - 全量作品 JSON payload。
  - 多章节快照数组。
  - 作品大纲 / 摘要 / 下一步提示拼接的 `globalOutline`。
- 旧单章作品没有 `chapters` 时，会自动转换为第一章快照。
- 成功后写入最近备份记录：“云端快照：作品名 · 时间”。
- 失败时明确提示后端原因，并引导先下载本地 JSON 备份。
- 新增“从云端恢复”入口：
  - 调用 `GET /api/work-library` 拉取当前账号作品快照。
  - 自动选择更新时间最新的一份。
  - 合并快照 payload 和后端章节数组。
  - 恢复为新的 `local-draft` / `local-only` 本地作品副本。
  - 标记标题为“云端恢复”，并追加“云端恢复”标签。
  - 不覆盖当前作品。
- 新增“查看云端快照”入口：
  - 列出当前账号所有后端作品快照。
  - 展示作品标题、更新时间、章节数和前端作品 ID。
  - 支持选择任意一份快照恢复为本地副本。
  - 支持刷新云端快照列表。
- 发布到自定义平台的按钮文案改为“准备文案”，避免误导为真实自动发文。

边界说明：

- 当前是手动云端快照和手动恢复，不是实时多端同步。
- 当前不做自动冲突合并，也不自动覆盖其他设备作品。
- 当前仍不接真实平台 OAuth，不自动发文。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。
- 已使用 JDK 21 启动后端服务，`http://localhost:8080` 正常监听。
- 已重新启动前端开发服务，`http://localhost:3000/` 返回 200。
- 已通过本地 HTTP 烟测验证 `POST /api/work-library` 保存快照、`GET /api/work-library` 拉取快照均返回 200，并包含 payload 与章节数据。
- 已通过本地 HTTP 烟测验证同一账号下两个云端作品快照可被列表接口返回，前端可据此展示并指定恢复。

## 2026-07-08 备份导入恢复本地闭环 P0

任务 ID：`NOVELAI-COPILOT-BACKUP-RESTORE-LOCAL-P0`

本轮先处理“已存在入口但仍只做校验提示”的本地闭环缺口：导出 / 备份中心的“导入备份文件”。目标是让用户下载 JSON 备份后，能在当前浏览器恢复为一个新的本地作品，而不是只看到“已读取”的占位提示。

已完成：

- `ExportCenterPage` 新增 `onRestoreWork` 回调，用于把导入结果交给主应用作品库。
- 导入备份支持解析：
  - 导出中心 `yixie-backup-v1` 包装格式。
  - 作品库导出的 `{ work }` / `{ works }` 结构。
  - 直接保存的单个作品对象。
- 导入后不会覆盖当前作品，而是生成新的 `local-draft` / `local-only` 本地作品副本。
- 导入时会补齐章节数组、单章兼容字段、字数、章节数、更新时间、标签、资料摘要和下一步提示。
- 备份中心文案从“只做文件校验与提示”改为“恢复为新的本地作品；不会覆盖当前作品”。
- 下载备份时如果没有当前作品，会提示先新建或打开作品，避免导出空备份。

边界说明：

- 当前仍不是云端恢复，不跨设备同步。
- 导入不会恢复模型设置到当前设置页，只恢复作品主体数据。
- 导入不会自动覆盖已有作品，避免误删或误改用户当前内容。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。

## 2026-07-08 发布准备本地草稿 P1

任务 ID：`NOVELAI-COPILOT-PUBLISH-PREP-LOCAL-P1`

本轮继续处理导出 / 备份 / 发布中心里的显性占位：原“生成发布简介”和“生成标签”只弹出成功提示，不产生可复用内容。现在改为本地生成可复制的发布准备草稿。

已完成：

- 发布中心新增“发布准备草稿”结果区。
- “生成发布简介”会基于当前作品题材、卖点、简介、章节标题、章节正文和下一步提示生成：
  - 作品简介。
  - 当前章节摘要。
  - 推广短句。
  - 下一步更新方向。
- “生成标签”会基于题材、标签、人物和卖点关键词生成平台检索标签。
- 发布准备草稿支持一键复制。
- 没有当前作品时会提示先新建或打开作品。
- 文案明确：本地生成，不会自动发布到任何平台。

边界说明：

- 当前仍不接真实发布平台，不自动发文。
- 当前标签和简介由本地规则生成，发布前需要人工删改。
...
## 2026-07-07 资料库后端同步闭环 P1

任务 ID：`NOVELAI-COPILOT-LORE-BACKEND-SYNC-P1`

本轮补齐创作工作台资料库与既有后端 Lore CRUD 的渐进式同步链路，不重写资料库 UI，不引入向量检索，不改变游客 / 本地草稿的本地闭环。

已完成：

- 前端 `SavedWork` 新增 `backendNovelId`，用于记录作品快照对应的后端 `novel.id`。
- 前端读取 `/api/work-library` 时会保留后端 `novelId`，打开后端作品后资料库可定位到真实后端小说。
- 保存正式作品快照时会读取后端返回的 `novelId`，并写回当前作品状态，方便后续资料库同步。
- 写作工作台资料库在登录、正式作品且已有 `backendNovelId` 时，会从 `/api/lore/list` 读取后端 Lore。
- 资料新增 / 编辑会在可同步时调用 `/api/lore` 的 POST / PUT；同步失败时保留本地资料，不阻塞写作。
- 资料删除会在可同步时调用 `/api/lore/{id}` DELETE；失败时先完成本地删除并提示同步失败。
- 增加前端 Lore 类型与后端 category 的映射：作品、人物、世界规则、地点、物品、势力、伏笔、章节摘要。
- 修复 `frontend/package.json` 中写死到不存在 Node 路径的问题，恢复为跨机器可用的 `next dev/build/start`。

边界说明：

- 当前 Lore 仍只同步后端已有的基础字段：分类、名称、内容；标签和关联章节仍是前端本地增强字段。
- 当前不接入 pgvector / embedding，不做真实向量召回。
- 没有 `backendNovelId` 的本地草稿和未保存正式作品仍使用本地资料库；保存作品后才进入后端资料同步路径。

验证：

- 已运行 `frontend` 下 `npm run build`，通过。
- 当前环境没有可用 `mvn` 命令，项目脚本中记录的固定 Maven 路径也不存在，因此本轮未能执行后端编译；本轮主要复用既有后端 Lore API，未修改 Java 代码。
